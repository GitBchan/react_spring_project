const ResultModal = ({title, content, callbackFn}) => {
    ...
    return (
        <div className={`fixed top-0 left-0 z-[1055] flex h-full w-full justify-center bg-black bg-opacity-20`} onClick={() => {
            ...
            <div className='absolute bg-white shadow dark:bg-gray-700 opacity-100 w-14 rounded mt-10 mb-10 px-6 min-w-[600px]'>
                ...
                <div className='justify-end flex'>
                    <button className='rounded bg-blue-500 mt-4 mb-4 px-6 pt-4 pb-4 text-lg text-white' onClick={() => {
                        if(callbackFn) {
                            callbackFn()
                        }
                    }}>Close Modal</button>
